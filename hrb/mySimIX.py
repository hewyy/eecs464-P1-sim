...
class RobotSimInterface( object ):
  ...
  def __init__(self, fn=None):
    ...
    c = mean(self.laserAxis,0)
    self.posY = vstack((c, c+asarray([0, -100])))
...
class SimpleRobotSim( RobotSimInterface ):
    ...
    def basewheels(self,dist,wheel):
        ...
        if wheel == 0:
            ...
            self.pos += -wheel * dot(delta_pos*array([cos(theta/2),sin(theta/2)]), [1,1j])
    
    def pulley(self,dist):
        # Move tag along the rope
        angle = self.getangle(self.ang) + pi/2
        self.pos += exp(1j*angle) * (dist + randn()*self.dNoise*sqrt(abs(dist)))
    ...
    def refreshState(self):
        ...
        # Visualize laser
        vl = array([c, (dot(self.laserAxis[0], [1,1j])-c)*150+c])
        # Calibration line
