...
from numpy import array, size, ones, hstack, vstack
...
from waypointShared import (
    ref, fitHomography, corners, waypoints, ROBOT_TAGID
)
...
class SimpleRobotSim( RobotSimInterface ):
    def __init__(self, *args, **kw):
        ...
        # FIXME
        self.dNoise = 0 # Distance noise 0.1
        self.aNoise = 0 # Angle noise 0.02
        self.lNoise = 0 # Laser pointing noise 0.01
        
        # Model the tag in arena coordinates
        # tag = dot(self.tagPos,[1,1j])
        tag_a = self.CameraToArena(self.tagPos)
        self.zTag = tag_a-mean(tag_a)
        self.base_a = mean(tag_a)
        self.pos = mean(tag_a)
        self.ang = self.initAng()
        self.tang = self.initAng()
    ...
    def initAng(self): # calculate the initial ang from tagPos
        cp = dot([[1,1,0,0],[0,0,1,1]],self.tagPos)/2
        angle = self.getangle(dot((cp[0] - cp[1]), [1,1j])) + pi/2
        return exp(1j*angle)
    ...
    def pulley(self,dist):
        ...
        if norm(self.base_a - temp) > self.basewidth/2:
            flag = sign((self.base_a - temp).imag)
    ...
    def basePos(self):
        tagcorner = self.zTag * self.ang + self.base_a
        pos = c_[tagcorner.real,tagcorner.imag]
        axis = dot([[1,1,0,0],[0,0,1,1]],pos)/2
        L = self.basewidth/2
        angle = self.getangle(self.ang)
        
        
        basePos = asarray([[axis[0, 0]-L*sin(angle), axis[0, 0]+L*sin(angle),
                            axis[1, 0]+L*sin(angle), axis[1, 0]-L*sin(angle)],
                           [axis[0, 1]+L*cos(angle), axis[0, 1]-L*cos(angle),
                            axis[1, 1]-L*cos(angle), axis[1, 1]+L*cos(angle)]])
        return basePos.T
    ...
    def refreshState(self):
        ...
        axis_a = dot([[1,1,0,0],[0,0,1,1]],tagPos_a)/2 #(2,2)
        basePos_a = self.basePos() #(4,2)
        # Transform back to camera coordinates
        tag = self.ArenaToCamera(tagPos_a)
        self.tagPos = c_[tag.real,tag.imag]
        axis = self.ArenaToCamera(axis_a)
        self.laserAxis = c_[axis.real,axis.imag]
        base = self.ArenaToCamera(basePos_a)
        self.base = hstack((base, base[0])) #(5,) closed rectangle
        c = mean(tag) # center
        cPos = mean(self.tagPos,0)
        self.posY = vstack((cPos, cPos+asarray([0, -100])))
        # Visualize laser
        vl = array([c, (axis[0]-c)*150+c])
        # Calibration line
        r = mean(abs(tag-c))
        ax = self.ang/self.tang*-1j
        cl = c + asarray([0,ax*100*r]) 
        ...
        self.visRobot('~plot',
            ...
            c='g')
        self.visRobot('~plot',
            [int(u) for u in cl.real],
            [int(u) for u in cl.imag],
            c='y',alpha=0.3)
        self.visRobot('~plot',
