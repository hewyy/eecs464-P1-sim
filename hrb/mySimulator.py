...
class AutoMode( Plan ):
    ...
    def limit(self,dist):
        # Limit the maximum stepsize
        if dist > 0:
            return min(dist, dist / float(self.N))
        else:
            return max(dist, dist / float(self.N))
    ...
    def behavior(self):
        ...
        while self.waypoints:
            ...
            while self.target == self.sensor.lastWaypoints[1][1]:
                ...
                # calculate the dist and step before the start of each move
                self.xstep = self.limit(self.target[0]-self.pos[0])
                self.ystep = self.limit(self.target[1]-self.pos[1])
                ...
                # calibrate the laser orientation
                pose = s.feedback()
                while abs(pose[1]) > self.err:
                    s.calibration(sign(self.xstep), self.err)
                    yield self.forDuration(0.5)
                    pose = s.feedback()
                
                # update the current position
                self.pos = pose[0]
                progress( "Current Position: [%0.2f, %0.2f]" % (self.pos[0], self.pos[1]) )
            
            # make sure the list of lastWaypoints get updated
            while not len(self.sensor.lastWaypoints[1])-1 == self.waypoints - 1:
  ...
  def onEvent( self, evt ):
    ...
    if evt.type == KEYDOWN:
      ...
      if evt.key == K_g:
        ...
        self.auto.start()
        return progress("(say) All Waypoints Reached. Stop Moving.")
    ### DO NOT MODIFY -----------------------------------------------
